            ...
            <article className='space-y-20'>
                    ...
                    <div className='lg:col-span-6 text-white space-y-8 '>
                        ...
                        <div className='space-y-4'>
                            ...
                            <div className='flex flex-wrap gap-x-10 gap-y-2'>
                                <Link onClick={() => window?.scrollTo({ top: 0, behavior: 'smooth' })} to={'consultant-signup'} className='bg-gradient-to-r from-blue-700 via-primary to-[#9544EB] bg-clip-text hover:text-secondary transition-all duration-500'>Astrologer Signup</Link>
                                <button onClick={() => dispatch(AuthActions.requestToggleAstrologerLoginModal())} className='bg-gradient-to-r from-blue-700 via-primary to-[#9544EB] bg-clip-text hover:text-secondary transition-all duration-500'>Astrologer login</button>
                            </div>
                        </div>
                    </div>
                ...
                <div className='max-w-6xl mx-auto px-4'>
                    <div className='flex flex-wrap justify-center gap-2 sm:gap-4'>
                        {['About Us', 'Terms of Use', 'Privacy Policy', 'Refund & Cancellation Policy'].map((label, idx, arr) => (
                            <React.Fragment key={label}>
                                <Link to={`/${label?.toLowerCase()?.split(' ')?.join('-')}`} className='text-gray-300 text-[13px] hover:text-secondary transition-all duration-500'>{label} </Link> {idx < arr.length - 1 && (<span className='text-gray-600'>|</span>)}
                            </React.Fragment>
                        ))}
                    </div>
