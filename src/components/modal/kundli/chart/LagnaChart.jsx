...
const LagnaChart = ({ data }) => {
    console.log("LagnaChart Data ::: ", data?.chart);
                            ...
                            return (
                                <g key={rashiIndex} className="sign">
                                    ...
                                    {planets?.map((planet, i) => (
                                        <g key={i}>
                                            {/* Planet Name */}
                                            <text x={textPosition.x + (i * 15) - 20} y={textPosition.y - 12} textAnchor="middle" fontSize="8" fill={planet?.color}>
                                                {planet?.name} {planet?.retrograde && '*'} {planet?.combust && '^'}
                                            </text>
                                            {/* Degree Value - Positioned Slightly Above */}
                                            <text x={textPosition.x + (i * 15) - 20} y={textPosition.y - 20} textAnchor="middle" fontSize="5" fontWeight="bold" >
                                                {planet?.degree?.deg}°
                                            </text>
                                        </g>
                                    ))}
