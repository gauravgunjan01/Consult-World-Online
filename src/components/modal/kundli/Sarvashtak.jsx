...
const Sarvashtak = ({ data }) => {
    console.log('Sarvashtak', data);
    ...
    return (
        <div className='overflow-x-scroll'>
            ...
            <div className=''>
                {selectedTab === 'Table' && <TableTab data={data?.sarvashtakaList} />}
                {selectedTab === 'Chart' && <div className='pt-10 pb-20'><ChartTab data={data?.chart} /></div>}
            </div>
        </div>
    );
}
...
const TableTab = ({ data }) => {
    const kundliAstakVargaData = data
    ...
    return (
        <>
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody>
                    {kundliAstakVargaData[0]?.prastaraks?.map((value, index) => {
                        return <tr>
                            <td className="border border-gray-200 px-4 py-2">{value?.name}</td>
                            {value?.sarvashtak?.map((curr, idx) => (
...
const ChartTab = ({data}) => {
    ...
    const kundliSarvashtakChartData = data
    
    console.log('kundliSarvashtakChartData ::: ', kundliSarvashtakChartData);
            ...
            <div className='flex flex-col gap-5'>
                ...
                <div className='flex-1 justify-center'>
                    <svg width="350" height="350" viewBox="0 0 350 350" className="w-[60%] h-full m-auto">
                        ...
                        { kundliSarvashtakChartData?.map((signData, index) => {
