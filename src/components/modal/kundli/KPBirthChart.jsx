...
const KPBirthChart = ({ data }) => {
    console.log("KPBirthChart Data ::: ", data?.chart);
                                ...
                                <g key={rashiIndex} className="sign">
                                    ...
                                    {/* {planets?.map((planet, i) => (
                                        <text key={i}  x={textPosition.x + (i * 13) - 20} y={textPosition.y - 12} textAnchor="middle" fontSize="8" fill={planet?.color}>
                                            {planet?.name} {planet?.degree?.deg}
                                        </text>
                                    ))} */}
                                    {planets?.map((planet, i) => (
                                        <g key={i}>
                                            {/* Planet Name */}
                                            <text x={textPosition.x + (i * 25) - 20} y={textPosition.y - 12} textAnchor="middle" fontSize="8" fill={planet?.color}>
                                                {planet?.name}
                                            </text>
                                            {/* Degree Value - Positioned Slightly Above */}
                                            <text x={textPosition.x + (i * 25) - 20} y={textPosition.y - 20} textAnchor="middle" fontSize="7" fontWeight="bold" fill={planet?.color}>
                                                {planet?.degree?.deg}°
                                            </text>
                                        </g>
                                    ))}
